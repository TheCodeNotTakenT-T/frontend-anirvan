  ...
  const NavItem = ({ view, label }: { view: ViewState, label: string }) => (
    <button
      ...
      className={`px-5 py-2 rounded-full text-sm font-bold transition-all duration-300 ${
        currentView === view 
          ? 'bg-Govan-accent text-Govan-dark shadow-lg shadow-Govan-accent/20' 
          : 'text-Govan-muted hover:text-white hover:bg-white/5'
      }`}
    >
      {label}
    </button>
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 flex justify-center px-4 ${scrolled ? 'pt-2' : 'pt-6'}`}>
      <nav className={`
        ...
        bg-Govan-dark/40 backdrop-blur-xl border border-white/10 shadow-2xl
      ...
      `}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer group" onClick={() => setView('landing')}>
            ...
            <span className="text-xl font-black tracking-tighter text-white hidden sm:block">Govan</span>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-1 bg-black/40 p-1 rounded-full border border-white/5 mx-4">
           ...
           <div className="hidden md:flex items-center gap-3">
            <div className="relative transition-all duration-300 w-64 focus-within:w-108 group">
                <Search className="absolute left-3 top-2.5 h-3.5 w-3.5 text-Govan-muted group-focus-within:text-Govan-accent z-10" />
                <input 
    type="text" 
    placeholder="Search blocks, accounts..." 
    className="bg-black/40 border border-white/5 rounded-full pl-9 pr-4 py-2 text-sm text-gray-300 focus:outline-none focus:border-Govan-accent/50 w-full transition-all"
/>
            </div>
          ...
          <div className="lg:hidden flex items-center gap-3">
             ...
             <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-2 text-Govan-muted hover:text-white">
